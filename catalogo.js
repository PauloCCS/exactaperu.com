...
function renderCatalog(){
 ...
 if(requested){
   ...
   if(target){document.querySelectorAll('.chip').forEach(x=>x.classList.remove('active'));target.classList.add('active');apply();}
 }
}
...
async function init(){menu();try{const r=await fetch('productos.json?v=31');if(!r.ok)throw new Error();PRODUCTS=await r.json()}catch(e){console.error('No se pudo cargar productos.json')}renderBrands();renderCategories();renderCatalog();renderProduct()}
